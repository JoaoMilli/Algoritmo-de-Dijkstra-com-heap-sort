...
Graph *leEntrada(FILE* file, int* servidor, int*cliente, int *monitor , int *v, int dadosVertice[][2], Graph *graph, int *nVert, int *nEdge, int *nServ, int *nClient, int *nMonitor)
{
	...
	for (int i = 0; i < *nEdge; i++)
	{
		...
		if (dadosVertice[aux_id1][0] == -1)
		{
			...
			v[aux] = aux_id1;
			Item * it = make_item(aux_id1, aux_id1);
...
int main (int argc, char ** argv) {
	...
	Item * result = calculaRTT(graph, nVert, nServ, nClient, nMonitor);
	...
	double **matriz = (double **)malloc(sizeof(double *) * nVert);

	for (int i = 0; i < nServ; i++){
		matriz[v[i]] = (double *)malloc(sizeof(double) * nVert);
		dijkstra(graph, v[i], matriz[v[i]]);
	}

	for (int i = nServ; i < nServ + nCliente; i++){
		matriz[v[i]] = (double *)malloc(sizeof(double) * nVert);
		dijkstra(graph, v[i], matriz[v[i]]);
	}
	
	for (int i = nServ + nCliente; i < nServ + nCliente + nMonitor; i++){
		matriz[v[i]] = (double *)malloc(sizeof(double) * nVert);
		dijkstra(graph, v[i], matriz[v[i]]);
	}
	*/
	...
	// double **servidor = (double **)malloc(sizeof(double *) * nVert);
	
	// for (int i = 0; i < nVert; i++){
	// 	servidor[i] = (double *)malloc(sizeof(double) * nVert);
	// }

	// for(int i=0; i < nVert; i++){
	// 	// printf("%d\n", i);
	// 	dijkstra(graph, i, servidor[i]);
	// }
}
